...
class Cli {
  ...
  createMotorbike(): void {
    inquirer
      ...
      .then((answers) => {
        // Create the wheels based on user input
        ...
        // Create the motorbike with the wheels
  ...
  performActions(): void {
    ...
    // Check the type of the selected vehicle to determine available actions
    const isTruck = selectedVehicle instanceof Truck;
    const isMotorbike = selectedVehicle instanceof Motorbike;

    // Create the choices array based on the vehicle type
    let choices = [
      'Print details',
      'Start vehicle',
      'Accelerate 5 MPH',
    ...
    // Add tow action if the vehicle is a truck
    if (isTruck) {
      choices.splice(8, 0, 'Tow a vehicle');
    }

    // Add wheelie action if the vehicle is a motorbike
    if (isMotorbike) {
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        } else if (answers.action === 'Tow a vehicle' && isTruck) {
          // find the selected vehicle (which is a truck) and call the findVehicleToTow method
        ...
        } else if (answers.action === 'Do a wheelie' && isMotorbike) {
          // find the selected vehicle (which is a motorbike) and do a wheelie
          const motorbike = selectedVehicle as Motorbike;
          motorbike.wheelie();
